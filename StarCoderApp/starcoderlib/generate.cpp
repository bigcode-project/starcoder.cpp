...
extern const float generate(const StarcoderModel * starcoder, const char * prompt, void (*token_callback)(const char * token)) {
    ...
    gpt_vocab vocab = starcoder->vocab;
    ...
    params.n_predict = std::min(params.n_predict, starcoder->model.hparams.n_ctx - (int) embd_inp.size());
    ...
    starcoder_eval(starcoder->model, params.n_threads, 0, { 0, 1, 2, 3 }, logits, mem_per_token);
    ...
    for (int i = embd.size(); i < embd_inp.size() + params.n_predict; i++) {
        ...
        if (embd.size() > 0) {
            ...
            if (!starcoder_eval(starcoder->model, params.n_threads, n_past, embd, logits, mem_per_token)) {
        ...
        if (i >= embd_inp.size()) {
            ...
            const int n_vocab = starcoder->model.hparams.n_vocab;
        ...
        if (starcoder->model.hparams.n_layer <= 30 && embd.back() == 49152) {
            break;
        }
